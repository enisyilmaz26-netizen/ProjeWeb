...
function WorkshopCard({ ws, language, cities, workshopRegistrations, workshopRegistrationsAvailable, loggedInUser, registering, today, onRegister, onUnregister, onOpenCertificate }) {
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            {isRegistered && (
              <span className="text-[10px] bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-2 py-0.5 rounded-lg font-medium flex-shrink-0 inline-flex items-center gap-0.5">
                <CheckCircle2 className="w-3 h-3" aria-hidden="true" />{t('workshop_registered_badge', language)}
              </span>
            )}
          </div>
          {ws.description && (
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {ws.date && (
              <span className="text-xs bg-[#1565C0]/10 dark:bg-[#7DD4FC]/10 text-[#1565C0] dark:text-[#7DD4FC] px-2 py-0.5 rounded-lg font-medium inline-flex items-center gap-1">
                <Calendar className="w-3.5 h-3.5 inline" aria-hidden="true" />{formatDate(ws.date)}
              </span>
            )}
            {ws.time && (
              <span className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-2 py-0.5 rounded-lg font-medium inline-flex items-center gap-1">
                <Clock className="w-3.5 h-3.5 inline" aria-hidden="true" />{ws.time}
              </span>
            )}
            {ws.capacity && (
              <span className={`text-xs px-2 py-0.5 rounded-lg font-medium inline-flex items-center gap-1 ${isFull ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300'}`}>
                <Users className="w-3.5 h-3.5 inline" aria-hidden="true" />{regCount}/{ws.capacity}
              </span>
            )}
          </div>
          {isPast && isRegistered && (
            <div className="mt-3">
              {hasAttended ? (
                ...
                <p className="text-center text-xs text-gray-400 dark:text-gray-500 py-1">
                  {t('awaiting_attendance', language)}
                </p>
              )}
            </div>
          )}
...
export default function WorkshopsScreen() {
  ...
  return (
    <div className="px-4 py-4 space-y-4">
      ...
      {regMsg && (
        <div role="status" aria-live="polite" className={`px-4 py-3 rounded-xl text-sm border ${regIsError ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800 text-red-700 dark:text-red-300' : 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 text-green-700 dark:text-green-300'}`}>
          {regMsg}
        </div>
      )}
          ...
          {upcoming.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-xs font-bold text-[#1565C0] dark:text-[#7DD4FC] uppercase tracking-wide">
                {t('upcoming_workshops', language)}
              </h3>
              {upcoming.map(ws => <WorkshopCard key={ws.id} ws={ws} language={language} cities={cities} workshopRegistrations={workshopRegistrations} workshopRegistrationsAvailable={workshopRegistrationsAvailable} loggedInUser={loggedInUser} registering={registering} today={today} onRegister={handleRegister} onUnregister={handleUnregister} onOpenCertificate={openCertificate} />)}
            </div>
          )}
          ...
          {past.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wide">
                {t('past_workshops', language)}
              </h3>
              {past.map(ws => <WorkshopCard key={ws.id} ws={ws} language={language} cities={cities} workshopRegistrations={workshopRegistrations} workshopRegistrationsAvailable={workshopRegistrationsAvailable} loggedInUser={loggedInUser} registering={registering} today={today} onRegister={handleRegister} onUnregister={handleUnregister} onOpenCertificate={openCertificate} />)}
            </div>
          )}
        </>
      )}
