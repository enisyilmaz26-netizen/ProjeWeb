...
export default function StudiosTab({ language, isGlobal, adminCityId, onRequestConfirm }) {
  ...
  const handleAddLab = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setLabSaveSuccess(t('record_added', language))
  ...
  const handleUpdateLab = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setLabSaveSuccess(t('record_updated', language))
        clearTimeout(successTimerRef.current); successTimerRef.current = setTimeout(() => setLabSaveSuccess(''), 3000)
      } else {
  ...
  const handleMigrateData = () => {
    onRequestConfirm(
      ...
      async () => {
        ...
        setMigrating(false)
        if (errors > 0) setLabError(t('migration_completed_errors', language).replace('{n}', errors))
      }
    )
  }
  ...
  return (
    <div>
        ...
        <div className="flex gap-2">
          {needsMigration && (
            <button onClick={handleMigrateData} disabled={migrating} className="text-xs text-orange-600 dark:text-orange-400 border border-orange-300 dark:border-orange-600 rounded-lg px-3 py-1.5 hover:bg-orange-50 dark:hover:bg-orange-900/20 transition disabled:opacity-60">
              {migrating ? '...' : <span className="inline-flex items-center gap-1"><RefreshCw className="w-3.5 h-3.5" aria-hidden="true" />{t('btn_fix_data', language)}</span>}
            </button>
          )}
      ...
      {isGlobal && (
        <select aria-label={t('filter_all_provinces', language)} className={`${inputClass} w-full mb-3`} value={labCityFilter} onChange={e => setLabCityFilter(e.target.value)}>
      ...
      {labSaveSuccess && (
        <div role="status" aria-live="polite" className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl px-4 py-3 text-green-700 dark:text-green-300 text-sm mb-3">{labSaveSuccess}</div>
      )}
      {labError && (
        <div role="status" aria-live="polite" className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl px-4 py-3 text-red-700 dark:text-red-300 text-sm mb-3">
          {labError} <button onClick={() => setLabError('')} aria-label={t('btn_close', language)} className="ml-2 text-red-400"><X className="w-3.5 h-3.5 inline" aria-hidden="true" /></button>
        </div>
      )}
      ...
      {showAddLab && (
        <form onSubmit={handleAddLab} className="bg-white dark:bg-[#0D1E3D] rounded-2xl shadow p-4 mb-4 space-y-3">
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={addLabLoading} className="flex-1 py-2 bg-[#1565C0] dark:bg-[#7DD4FC] text-white dark:text-[#060E26] text-xs font-semibold rounded-xl disabled:opacity-60">{addLabLoading ? '...' : t('btn_save', language)}</button>
              ...
              {isEditing ? (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex gap-1 flex-shrink-0">
                    <button onClick={() => startEditLab(lab)} aria-label={t('btn_edit', language)} title={t('btn_edit', language)} className="text-[#1565C0] dark:text-[#7DD4FC] text-xs p-1.5 hover:bg-[#1565C0]/10 rounded-lg transition"><Pencil className="w-3.5 h-3.5" aria-hidden="true" /></button>
                    <button onClick={() => handleDeleteLab(lab.id)} disabled={processingId === lab.id} aria-label={t('btn_delete', language)} title={t('btn_delete', language)} className="text-red-500 hover:text-red-700 text-xs p-1.5 disabled:opacity-40"><Trash2 className="w-3.5 h-3.5" aria-hidden="true" /></button>
                  </div>
                </div>
              )}
            </div>
